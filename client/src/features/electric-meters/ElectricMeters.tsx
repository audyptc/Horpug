...
import { ElectricMeterDialog, type ElectricMeterForm } from '@/features/electric-meters/components/ElectricMeterDialog'
...
export function ElectricMeters() {
  const { t } = useTranslation()
  const { canCreate, canUpdate, canDelete } = usePermission('/electric-meters')

  const [readings, setReadings] = useState<ApiElectricMeter[]>([])
  ...
  const [editing, setEditing] = useState<ApiElectricMeter | null>(null)
  const [deleting, setDeleting] = useState<ApiElectricMeter | null>(null)
  ...
  const fetchReadings = useCallback(async (p: number, pp: number) => {
    ...
    setError('')
    try {
      const resp = await electricMeterService.list(p, pp)
      setReadings(resp.data)
      setTotal(resp.meta.total)
      setTotalPages(resp.meta.total_pages)
    } catch {
      setError(t('electricMeters.loadError'))
    } finally {
      setLoading(false)
    }
  }, [t])
  ...
  function openEdit(r: ApiElectricMeter) {
    setEditing(r)
    setForm({
      room_id: r.room_id,
  ...
  async function handleDelete() {
    if (!deleting) return
    try {
      await electricMeterService.delete(deleting.id)
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <h1 className="text-2xl font-bold tracking-tight">{t('electricMeters.title')}</h1>
          <p className="text-muted-foreground text-sm mt-1">{t('electricMeters.subtitle', { count: filtered.length, total })}</p>
        </div>
        <div className="flex gap-2 shrink-0">
          ...
          {canCreate && (
            <Button onClick={openCreate} className="gap-2">
              ...
              <span className="hidden sm:inline">{t('electricMeters.addReading')}</span>
            </Button>
          )}
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <CardTitle>{t('electricMeters.readingList')}</CardTitle>
              <CardDescription>{t('electricMeters.readingListDesc')}</CardDescription>
            </div>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
              <Input placeholder={t('electricMeters.searchPlaceholder')} className="pl-9 w-full sm:w-48"
            ...
            <>
              <div className="hidden md:block overflow-x-auto">
                ...
                {filtered.length === 0 && (
                  <div className="flex flex-col items-center justify-center py-16 gap-3 text-muted-foreground">
                    ...
                    <p className="text-sm font-medium">{t('electricMeters.noReadings')}</p>
                    {search && (
                      <button type="button" onClick={() => setSearch('')} className="text-xs text-primary hover:underline">
                        {t('electricMeters.clearFilters')}
                      </button>
                    )}
                  </div>
                )}
              ...
              {!loading && total > 0 && (
                <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-4 py-3 border-t text-sm text-muted-foreground">
                  <span className="shrink-0">{t('electricMeters.showing', { from: (page - 1) * perPage + 1, to: Math.min(page * perPage, total), total })}</span>
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                      <span className="hidden sm:inline shrink-0">{t('electricMeters.perPage')}</span>
                      <Select value={String(perPage)} onValueChange={(v) => { setPerPage(Number(v)); setPage(1) }}>
                        <SelectTrigger className="h-8 w-16"><SelectValue /></SelectTrigger>
                        <SelectContent>{PER_PAGE_OPTIONS.map((n) => <SelectItem key={n} value={String(n)}>{n}</SelectItem>)}</SelectContent>
                      </Select>
                    </div>
                    <span className="shrink-0">{t('electricMeters.page')} {page} {t('electricMeters.of')} {totalPages}</span>
      ...
      <ElectricMeterDialog open={dialogOpen} onOpenChange={setDialogOpen} editing={editing}
        form={form} onFormChange={setForm} onSave={handleSave} saving={saving} rooms={rooms} />
      <ElectricMeterDeleteDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}
