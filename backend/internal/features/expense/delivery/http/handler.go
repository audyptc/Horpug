...
func parseColumnFilters(c fiber.Ctx) (map[string]string, error) {
	...
	for key, value := range c.Queries() {
		...
		if _, ok := expenseusecase.FilterColumns[name]; !ok {
...
func parseListFilters(c fiber.Ctx) (expenseusecase.ListFilters, error) {
	...
	if raw := strings.TrimSpace(c.Query("sort")); raw != "" {
		if _, ok := expenseusecase.SortColumns[raw]; !ok {
			return expenseusecase.ListFilters{}, apierror.BadRequest("unsupported sort field")
		}
		sortKey = raw
	}
	...
	sortDesc := sortKey == expenseusecase.DefaultSortKey
	...
	case "desc":
		sortDesc = true
	default:
		return expenseusecase.ListFilters{}, apierror.BadRequest("order must be asc or desc")
	}
...
// @Param q query string false "Filter by dormitory name or description"
// @Param f[column] query string false "Per-column substring filter, e.g. f[description]=repair; column must be one of dormitory_name, description"
// @Param sort query string false "Sort field: dormitory_name, category, expense_date, amount, description (default expense_date)"
