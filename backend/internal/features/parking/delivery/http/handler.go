...
func parseColumnFilters(c fiber.Ctx) (map[string]string, error) {
	...
	for key, value := range c.Queries() {
		...
		if _, ok := parkingusecase.FilterColumns[name]; !ok {
...
func parseListFilters(c fiber.Ctx) (parkingusecase.ListFilters, error) {
	...
	sortKey := parkingusecase.DefaultSortKey
	if raw := strings.TrimSpace(c.Query("sort")); raw != "" {
		if _, ok := parkingusecase.SortColumns[raw]; !ok {
			return parkingusecase.ListFilters{}, apierror.BadRequest("unsupported sort field")
		}
		sortKey = raw
	}
	...
	sortDesc := sortKey == parkingusecase.DefaultSortKey
	...
	case "desc":
		sortDesc = true
	default:
		return parkingusecase.ListFilters{}, apierror.BadRequest("order must be asc or desc")
	}
...
// @Param q query string false "Filter by tenant name, room number, dormitory name, license plate or parking spot"
// @Param f[column] query string false "Per-column substring filter, e.g. f[license_plate]=1กก; column must be one of tenant_name, room_number, license_plate, parking_spot"
// @Param sort query string false "Sort field: tenant_name, room_number, vehicle_type, license_plate, parking_spot, created_at (default created_at)"
