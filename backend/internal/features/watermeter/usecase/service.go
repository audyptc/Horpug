...
const IsBilledExpr = `(EXISTS (SELECT 1 FROM invoice_items ii WHERE ii.reference_id = wm.id AND ii.item_type = 'water') OR EXISTS (SELECT 1 FROM move_out_items mi WHERE mi.reference_id = wm.id))`
