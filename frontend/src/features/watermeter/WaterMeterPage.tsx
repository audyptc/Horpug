...
export default function WaterMeterPage() {
  ...
  const [billingMethodFilter, setBillingMethodFilter] = useState<WaterMeterBillingMethodFilter>('all')
  const [billedFilter, setBilledFilter] = useState<WaterMeterBilledFilter>('all')
  // Applied on submit from each column's menu, so no debounce is needed here.
  const [columnFilters, setColumnFilters] = useState<WaterMeterColumnFilters>({})
  // The list reads as a running log, so it opens on the newest readings.
  const [sortKey, setSortKey] = useState<WaterMeterSortKey>('reading_date')
  const [sortDirection, setSortDirection] = useState<WaterMeterSortDirection>('desc')

  const [page, setPage] = useState(1)
  const [pageSize, setPageSizeState] = useState<number>(WATER_METER_PAGE_SIZE_OPTIONS[0])
  ...
  useEffect(() => {
    ...
    api
      .get<ApiPage<ApiWaterMeter[]>>('/water-meters', {
  ...
  function handleSort(key: WaterMeterSortKey) {
    if (key === sortKey) {
  ...
  async function handleFormSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (!isEdit) {
        ...
        await api.post<ApiWaterMeter>('/water-meters', payload)
      } else {
        const payload = {
          billing_method: formBillingMethod,
          reading_date: toApiDate(formReadingDate),
        ...
        await api.put<ApiWaterMeter>(`/water-meters/${formMeterId}`, payload)
      }
      refresh()
      setFormOpen(false)
    } catch (err) {
  ...
  return (
    <main className="content">
      ...
      <WaterMeterListCard
        ...
        onColumnFilterChange={(key: WaterMeterTextFilterKey, value: string) => {
